...
def CardOfPartition(Candidate, df):
    ...
    # If length is +1, create groups over which to find number of unique elements
        ...
        return df.drop_duplicates(Candidate).count().iloc[0]
...
def f(C_km1, df, Closure, U, Cardinality):
    ...
    SubsetsToCheck = [list(Subset)
        for Subset in set([frozenset(Candidate + [v_i])
    ...
    for Candidate in C_km1:
        ...
        for v_i in list(set(U_c).difference(Closure[candidateBinRepr])):
            ...
            if Cardinality[candidateBinRepr] == Cardinality[binaryRepr.toBin(Candidate + [v_i], U)]:
                ...
                Closure[candidateBinRepr].add(v_i)
